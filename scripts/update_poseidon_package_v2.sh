...
function Helptext() {
  ...
  echo -ne "-r, --root_output_directory\t\tOptional. The root directory to place the output packages in. Default: '/mnt/archgen/Autorun_eager/dev/poseidon_packages'"
...
## Parse CLI args.
TEMP=`getopt -q -o ha:r:v --long help,analysis_type:,root_output_dir:version -n 'update_poseidon_package.sh' -- "$@"`
eval set -- "$TEMP"
...
if [[ -z "${analysis_type}" ]]; then 
  errecho -r "[update_poseidon_package.sh]: No analysis type was provided.\n"
  Helptext
  exit 1
fi
...
if [[ -z ${ind_id} ]]; then
  errecho -r "[update_poseidon_package.sh]: No individual ID provided.\n"
  Helptext
  exit 1
fi
...
# root_output_dir='~/poseidon_packages' ## Directory that includes data type, site ID and ind ID subdirs.
...
if [[ ! -d ${input_dir} ]]; then
  errecho -r "[update_poseidon_package.sh]: Expected eager output directory '${input_dir}' does not exist."
  exit 1
fi
...
if [[ ! -f ${input_dir}/pileupcaller.single.geno ]] && [[ ! -f ${input_dir}/pileupcaller.double.geno ]]; then
  errecho -r "[update_poseidon_package.sh]: No valid genotype files (*.geno) found in output directory."
...
## This will evaluate to TRUE when the newest AE geno is newer than the output geno, or when there is no output geno (i.e. no pacakge).
if [[ ${newest_geno} -nt ${output_dir}/*geno ]]; then
    TEMPDIR=$(mktemp -d ${autorun_root_dir}/.tmp/v2/${ind_id}_XXXXXXXX)
    errecho -y "[update_poseidon_package.sh]: Pulling genotypes for ${ind_id}."
  ...
  ${trident_path} init \
    ...
    2>&1 | tee -a ${LOG} ## Save stderr/stdout to creation.log file for future reference.
  ...
  check_fail $? "[update_poseidon_package.sh]: Failed to fill janno file information. See: ${LOG}"
